...
2. **文章详情页 (Details Page)**：需登录访问，包含可见度校验，完整文章内容展示、评论留言区、点赞与点踩功能、文章下载功能。
3. **用户认证与管理 (Authentication & Account)**：用户注册、用户/管理员登录、**账号注销（及文章资产导出提醒）**。
...
| 详情页 | 文章阅读区 | **必须登录且具有查看权限才能访问**，完整展示文章正文。 |
| 详情页 | 文章下载 | 提供下载按钮，支持将文章内容及元数据打包为 Markdown (`.md`) 文件下载到本地设备。 |
| 详情页 | 互动操作区 | 提供“点赞(Like)”和“点踩(Dislike)”按钮，点击可更新对应数值。 |
| 详情页 | 评论互动区 | 用户可输入内容并提交评论。 |
| 导航栏 | 账号操作 | 提供登录、注册入口；登录后提供退出(Logout)和**注销(Deregister)**入口。 |
| 注销弹窗 | 数据导出提醒 | 用户点击注销时，若名下有已发布或草稿文章，弹窗提示并展示文章列表。用户可勾选部分或全部文章，打包下载到本地后完成账号注销。 |
